...
c
      i=1
   10 continue
      read(5,*,end=20) alam(i),anum(i),gf(i),excl(i)
      i=i+1
      go to 10
   20 continue
      n=i-1
...
c
      ...
      ioldend=0
   30 continue
      read(10,*,end=50) al,anu,a,ex
      if(ioldend.eq.1) then
         write(11,611) al,anu,a,ex
         j=j+1
         go to 30
      end if
  ...
  621        format(2i10,2f15.4,3i10)
            if(k.gt.n) go to 70
            write(11,611) alam(k),anum(k),gf(k),excl(k)
            k=k+1
            go to 40
          else
            write(11,611) al,anu,a,ex
  ...
  611 format(f10.4,f10.2,f8.3,f12.3)
c
      close(11)
      end
